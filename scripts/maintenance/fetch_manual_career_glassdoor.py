...
If that is blocked, pass --browser --headed once with a persistent Chrome profile.
  ...
  ~/.v/bin/pip install curl_cffi
  ~/.v/bin/python fetch_manual_career_glassdoor.py --limit 10
  ~/.v/bin/python fetch_manual_career_glassdoor.py --workers 8
  ~/.v/bin/python fetch_manual_career_glassdoor.py --browser --headed --limit 5
"""
...
def main() -> int:
    ...
    parser.add_argument("--limit", type=int, default=0, help="Max companies to fetch (0 = all missing)")
    ...
    parser.add_argument(
        ...
        type=Path,
        default=PROFILE_DIR,
        help="Persistent Chromium profile directory (--browser)",
    )
    ...
    if args.browser:
        ...
        for idx, (cid, name) in enumerate(todo, 1):
            ...
            html = fetch_html_playwright(name, headed=args.headed, profile_dir=args.profile_dir)
    ...
    print(f"Done — {ok}/{len(todo)} updated. Cache: {qj.cache_dir(REPO_ROOT) / 'glassdoor'}")
